...
process {
	...
	if (-not (Test-Path -Path $runFile)) {
		throw "Target folder does not contain a run.ps1 file, which is used to actually execute the bootstrap code: $runFile"
	}
	...
	$bootstrapCode = [System.IO.File]::ReadAllText("$PSScriptRoot\bootstrap.ps1")
